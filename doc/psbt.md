# PSBT Howto for Aryacoin Core

Since Aryacoin Core 0.17, an RPC interface exists for Partially Signed Aryacoin
...
PSBT is an interchange format for Aryacoin transactions that are not fully signed
...
Overall, the construction of a fully signed Aryacoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Aryacoin transaction (in network format)
...
#### Multisig with multiple Aryacoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Aryacoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `aryacoin-cli` is used.
