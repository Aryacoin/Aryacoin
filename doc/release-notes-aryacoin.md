Aryacoin Core version 0.18.1 is now available from:

  <https://download.aryacoin.io/aryacoin-3.0.0/>
...
`/Applications/Aryacoin-Qt` (on Mac) or `aryacoind`/`aryacoin-qt` (on
Linux).
...
Aryacoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Aryacoin Core on unsupported systems.

Aryacoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Aryacoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/Aryacoin/Aryacoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@aryacoin.io) the Aryacoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 AYA),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in AYA per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/Aryacoin/Aryacoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/Aryacoin/Aryacoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/Aryacoin/Aryacoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/Aryacoin/Aryacoin/blob/master/doc/aryacoin-conf.md)
  about the `aryacoin.conf` file describes how to use it to configure
  Aryacoin Core.

- A new document introduces Aryacoin Core's BIP174 [Partially-Signed
  Aryacoin Transactions
  (PSBT)](https://github.com/Aryacoin/Aryacoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/Aryacoin/Aryacoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Aryacoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Aryacoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/Aryacoin/Aryacoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/Aryacoin/Aryacoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Aryacoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Aryacoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/Aryacoin/Aryacoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/Aryacoin/Aryacoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart aryacoind with the `-deprecatedrpc=generate`
...
- A new `aryacoin-wallet` tool is now distributed alongside Aryacoin
...
This section describes planned changes to Aryacoin Core that may affect
other Aryacoin software and services.

- Since version 0.16.0, Aryacoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Aryacoin Core 0.20 (expected about
  a year after 0.18), Aryacoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Aryacoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Aryacoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Aryacoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Aryacoin Core
...
- A sub-project of Aryacoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Aryacoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Aryacoin Core's own implementation, although entropy
  gathered by Aryacoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Aryacoin Core a
...
- On macOS, Aryacoin Core now opts out of application CPU throttling
